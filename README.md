...
## 更多实现
在后续实验中，我还进行了以下创新点的实验：（详情见小组部分报告）
- **动态Gamma调整**：根据草稿模型的被接受能力动态调整gamma值，以进一步优化速度和准确性之间的平衡。灵感来自于CS3611-计算机网络的丢包重传机制——AIMD，当模型近期表现不佳，说明对附近序列不自信，需要迅速减小gamma避免大量不接受。详情见小组报告，方法实现于specSampling.py/specSampling_new。
- **多阶段草稿模型**：引入多个草稿模型，形成多阶段的投机采样流程，以进一步提升推理效率和输出质量。灵感来自于ICE2603-计算机组成中的多级缓存设计。但最终实现效果不佳，分析与详情见小组报告，方法实现于specSampling.py/specSampling_new_multi。
- **结合KVpress**：通过结合KV压缩技术，进一步减少内存占用和计算开销，从而提升推理速度。KVpress部分由队友实现，整合部分代码见https://github.com/MooNknightO2/CS3602_FINAL_MIX.
